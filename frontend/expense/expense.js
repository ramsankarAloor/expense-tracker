...
async function getIncomesPerDate(date){
  ...
  const {data:incomes} = await axios.post("http://localhost:3000/expenses",{date:date, isIncome : true}, {
    headers : {
...
async function getExpensesPerDate(date){
  const token = localStorage.getItem('token');
  const {data:expenses} = await axios.post("http://localhost:3000/expenses",{date:date, isIncome:false}, {
    headers : {
      'Authorization' : `Bearer ${token}`
    }
  });
...
async function setupForPremium(){
  const token = localStorage.getItem('token');
  const {data:isUserPremium} = await axios.get('http://localhost:3000/isPremium', {
    headers : {
      'Authorization' : `Bearer ${token}`
    }
  });
...
function displayIncomeRecord(object){
  ...
  async function deleteIncome() {
    const token = localStorage.getItem('token');
    await axios.delete(`http://localhost:3000/incomes/${object.id}`, {
      headers : {
        'Authorization' : `Bearer ${token}`
      }
    });
...
async function postNewExpense() {
  ...
  await axios.post(
    "http://localhost:3000/new-expense", obj, {
      headers : {
        'Authorization' : `Bearer ${token}`
      }
    }
